...
from app.graphs.states import ModifyLocationState
...
from app.graphs.nodes.text import (
    build_location_modify_prompt, call_llm,
    parse_location_json_result, validate_location_description,
)


def create_modify_location_graph() -> StateGraph:
    graph = StateGraph(ModifyLocationState)

    graph.add_node("validate_task", validate_task)
    graph.add_node("build_modify_prompt", build_location_modify_prompt)
...
def _route_after_call(state: ModifyLocationState) -> str:
...
def _route_after_parse(state: ModifyLocationState) -> str:
    errors = state.get("errors", [])
...
def _route_after_validate(state: ModifyLocationState) -> str:
    errors = state.get("errors", [])
