...
from app.graphs.states import ModifyPropState
...
from app.graphs.nodes.text import (
    build_prop_modify_prompt, call_llm,
    parse_prop_json_result, validate_prop_description,
)


def create_modify_prop_graph() -> StateGraph:
    graph = StateGraph(ModifyPropState)

    graph.add_node("validate_task", validate_task)
    graph.add_node("build_modify_prompt", build_prop_modify_prompt)
    graph.add_node("call_llm", call_llm)
    graph.add_node("parse_json_result", parse_prop_json_result)
    graph.add_node("validate_description", validate_prop_description)
    ...
    graph.add_edge("validate_task", "build_modify_prompt")
    graph.add_edge("build_modify_prompt", "report_progress")
...
def _route_after_call(state: ModifyPropState) -> str:
...
def _route_after_parse(state: ModifyPropState) -> str:
    errors = state.get("errors", [])
...
def _route_after_validate(state: ModifyPropState) -> str:
    errors = state.get("errors", [])
