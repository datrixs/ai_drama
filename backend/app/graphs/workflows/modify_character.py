...
from app.graphs.states import ModifyCharacterState
...
from app.graphs.nodes.text import (
    build_modify_prompt, call_llm, parse_json_result, validate_description,
)


def create_modify_character_graph() -> StateGraph:
    graph = StateGraph(ModifyCharacterState)

    graph.add_node("validate_task", validate_task)
    graph.add_node("build_modify_prompt", build_modify_prompt)
    graph.add_node("call_llm", call_llm)
    graph.add_node("parse_json_result", parse_json_result)
    graph.add_node("validate_description", validate_description)
...
def _route_after_call(state: ModifyCharacterState) -> str:
...
def _route_after_parse(state: ModifyCharacterState) -> str:
    errors = state.get("errors", [])
...
def _route_after_validate(state: ModifyCharacterState) -> str:
    errors = state.get("errors", [])
