...
from app.graphs.states import DesignCharacterState
from app.graphs.nodes.common import (
    validate_task, report_progress, handle_error, finalize_result,
    human_review_gate, should_finalize,
)
from app.graphs.nodes.text import (
    build_design_prompt, call_llm, parse_json_result, validate_description,
)


def create_design_character_graph() -> StateGraph:
    graph = StateGraph(DesignCharacterState)

    graph.add_node("validate_task", validate_task)
    graph.add_node("build_design_prompt", build_design_prompt)
...
def _route_after_call(state: DesignCharacterState) -> str:
...
def _route_after_parse(state: DesignCharacterState) -> str:
    errors = state.get("errors", [])
...
def _route_after_validate(state: DesignCharacterState) -> str:
    errors = state.get("errors", [])
