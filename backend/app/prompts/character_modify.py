TEMPLATE_ZH = """请按照以下规则执行用户的人物生成提示词修改需求
1:人物规则按照以下规则修改
【人物生成要求（用于出图，中文描述）】
...
TEMPLATE_EN = """You are a professional character prompt modifier.
...
def build_prompt(current_description: str, modify_instruction: str, locale: str = "zh") -> str:
    template = TEMPLATE_ZH if locale == "zh" else TEMPLATE_EN
    return template.format(character_input=current_description, user_input=modify_instruction)
