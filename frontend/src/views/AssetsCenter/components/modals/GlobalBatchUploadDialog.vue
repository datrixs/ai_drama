    ...
    <div class="modal-dialog glass-surface-modal">
      ...
      <div class="dialog-config">
        <div class="folder-select">
          <span class="config-label">资产组</span>
          <el-select
          ...
          >
            <el-option
              v-for="f in assetHubStore.folders"
              :key="f.id"
              :label="f.name"
              :value="f.id"
            />
...
// 资产组下拉的本地值：null = 未分组
const selectedFolderId = ref(props.folderId || null)
const files = ref([])
...
async function uploadOne(item) {
  ...
  if (activeType.value === 'character') {
    const created = await assetHubStore.createCharacter(
      { name: item.name, folder_id: folderId },
      { silent: true },
    )
    // 后端 create_character 返回 {character, appearance} 嵌套结构
    const characterId = created?.character?.id || created?.id
    if (!characterId) throw new Error('创建失败')
    const res = await assetHubStore.uploadCharacterImage({
      file: item.file,
      characterId,
      appearanceIndex: 0,
      labelText: '',
    ...
    if (res?.sync_error) syncError = res.sync_error
  } else {
    // 场景 / 道具：底层共用 GlobalLocation 表，靠 asset_kind 区分
    const assetKind = activeType.value === 'prop' ? 'prop' : 'location'
    const created = await assetHubStore.createLocation(
      { name: item.name, folder_id: folderId, asset_kind: assetKind },
      { silent: true },
    )
    const locationId = created?.id
    if (!locationId) throw new Error('创建失败')
    const res = await assetHubStore.uploadLocationImage({
      file: item.file,
      locationId,
      imageIndex: 0,
      labelText: '',
...
async function onConfirm() {
  ...
  try {
    for (const item of files.value) {
      ...
      item.error = null
      try {
        const syncError = await uploadOne(item)
        item.status = 'done'
        ok++
        if (syncError) {
          syncErrors.push(`${item.name}：${syncError}`)
        }
      } catch (e) {
        item.status = 'failed'
    ...
    if (ok > 0) {
      // 统一刷新一次列表
      await Promise.all([assetHubStore.fetchCharacters(), assetHubStore.fetchLocations(), assetHubStore.fetchProps()])
      emit('done')
    }
  } finally {
...
watch(() => props.visible, (v) => {
  if (v) {
    ...
    selectedFolderId.value = props.folderId || null
    files.value = []
    isDragging.value = false
    uploading.value = false
    // 资产组列表兜底
    if (!assetHubStore.folders?.length) {
      assetHubStore.fetchFolders?.()
    }
  }
})
</script>
