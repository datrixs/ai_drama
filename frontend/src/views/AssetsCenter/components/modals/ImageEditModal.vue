    ...
    <div class="modal-card glass-surface-modal">
      ...
      <div class="modal-header">
        <div>
          ...
          <p class="modal-subtitle">{{ typeLabel }}: {{ assetName }} · 输入修改指令，可选择上传参考图片</p>
        </div>
      ...
      <div class="modal-body">
        ...
        <div class="form-field">
          ...
          <div class="ref-images-grid">
            <div v-for="(img, idx) in editImages" :key="idx" class="ref-thumb">
...
const props = defineProps({
  assetType: {
    type: String,
    required: true,
    validator: (v) => ['character', 'location', 'prop'].includes(v)
  },
...
async function handleConfirm() {
  if (!instruction.value.trim() || isSubmitting.value) return
  isSubmitting.value = true
  // 乐观更新：立即标记卡片为 generating
  if (props.assetType === 'character') {
    const char = store.characters.find(c =>
      (c.appearances || []).some(a => a.id === undefined || a.character_id === props.assetId)
    )
    if (char) {
      const app = char.appearances?.[props.imageIndex]
      if (app) app.gen_status = 'generating'
    }
  } else {
    const loc = store.locations.find(l => l.id === props.assetId)
    if (loc) {
      const img = (loc.images || [])[props.imageIndex]
      if (img) img.gen_status = 'generating'
    }
  }
  try {
    await submitAsyncTask(
      '/asset-hub/modify-image',
      {
        type: props.assetType,
        id: props.assetId,
        image_index: props.imageIndex,
        modify_prompt: instruction.value.trim(),
        extra_image_urls: editImages.value.length > 0 ? editImages.value : [],
      },
      { successMsg: 'AI 改图完成' }
    )
    if (props.assetType === 'character') {
